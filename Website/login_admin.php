...
$admin_name = $password = $voterid = $aadhar = "";
$err = "";

// if request method is post
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    if (empty(trim($_POST['admin_name'])) || empty(trim($_POST['password'])) || empty(trim($_POST['voterid'])) || empty(trim($_POST['aadhar']))) {
        $err = "Please enter admin_name + password + voterid + aadhar";
    ...
    if (empty($err)) {
        $sql = "SELECT id, admin_name, password ,voterid ,aadhar FROM admin WHERE aadhar = ?";
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            if (mysqli_stmt_num_rows($stmt) == 1) {
                mysqli_stmt_bind_result($stmt, $id, $admin_name, $hashed_password, $voterid, $aadhar);
                if (mysqli_stmt_fetch($stmt)) {
                    if (password_verify($password, $hashed_password)) {
                        ...
                        //Redirect user to profile_admin page
                        header("location: profile_admin.php");
...
<div class="navigation-wrap bg-light start-header start-style">
    ...
    <div class="container mt-4" style="padding : 130px 0px; ">
        <h3 style="font-size:30px ">Admin Login Here:</h3>
        <hr>

        <form action="" method="post">
            <div class="form-group" style="font-size:20px ">
                <label for="exampleInputadmin_name">Admin Name</label>
                <input type="text" name="admin_name" class="form-control" id="exampleInputadmin_name"
                    aria-describedby="emailHelp" placeholder="Enter Admin Name" required>
            </div>
            <div class="form-group" style="font-size:20px ">
